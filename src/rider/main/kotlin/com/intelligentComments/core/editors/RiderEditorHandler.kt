...
У людей всё иначе. Одни программы нам воспринимать легче, чем другие, и мы ищем комментарии, которые помогут нам разобраться."""
    ...
    val highlighter = RdTextHighlighter(
      "text.test.first.color",
      0,
      4,
      RdTextAttributes(),
    ...
    val highlighter1 = RdTextHighlighter(
      "text.test.second.color",
      200,
      213,
      RdTextAttributes(fontWeight = 1000f, fontStyle = RdFontStyle.Bold, underline = true),
      animation = RdUnderlineTextAnimation()
    )
    ...
    fun getListHeader(text: String): RdHighlightedText {
      ...
      val highlighter =
        RdTextHighlighter("text.default.color", 0, text.length, RdTextAttributes(), animation = animation)
    ...
    val listContent2 = RdListSegment(
      mutableListOf(RdContentSegments(mutableListOf(innerListContent, innerListContent2))),
      getListHeader("Inner list:")
    )
    ...
    val listContents = mutableListOf(
      RdContentSegments(mutableListOf(listContent1)),
      RdContentSegments(mutableListOf(listContent2)),
      RdContentSegments(mutableListOf(listSegment4)),
      RdContentSegments(mutableListOf(listContent3))
    )
    ...
    val imageSegment1 = RdFileBasedImageSegment(
      catPath,
      RdHighlightedText("This cat is awesome This cat is awesome This cat is awesome This cat is awesome This cat is awesome This cat is awesome")
    )
    ...
    fun getTableCellContent(): RdContentSegments {
      ...
      val list =
        RdListSegment(mutableListOf(RdContentSegments(mutableListOf(listText))), getListHeader("List inside table:"))
      val imageSegment = RdFileBasedImageSegment(catPath, RdHighlightedText("This cat is awesome"))
    ...
    val headerRows = RdTableRow(
      listOf(
        RdTableCell(
          getHeaderCellContent(),
          properties = RdTableCellProperties(RdHorizontalAlignment.Center, RdVerticalAlignment.Center, true)
        ),
        RdTableCell(
          getHeaderCellContent(),
          properties = RdTableCellProperties(RdHorizontalAlignment.Center, RdVerticalAlignment.Center, true)
    ...
    val rows = RdTableRow(
      listOf(
        RdTableCell(getTableCellContent()),
        RdTableCell(
          getTableCellContent(),
          properties = RdTableCellProperties(RdHorizontalAlignment.Right, RdVerticalAlignment.Top, false)
        )
      )
    )
...
This method depends on the synchronous nature of Method1,""".trimMargin(),
    ...
    val comment =
      RdIntelligentComment(authors, Date(), content, invariants, references, todos, null, 0, RdTextRange(0, 0))
  }
}